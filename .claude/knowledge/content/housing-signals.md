# Signal ledger: Homelessness and Housing

Every signal Charlie has ever mined, and what happened to it. Read before proposing, written
after approval.

Without this, week 4 re-proposes week 3's rejects and Kiki says no to the same topic twice.

**No prospect names, companies, emails or phone numbers in this file.** It is committed to a
deny-by-default repo whose stated purpose excludes client PII, and it is read by a cloud routine.
Roles and organisation types only.
...
|---|---|---|
| `observed` | Seen in the log, never put in front of Kiki | No. Eligible, and the history is useful. |
...
re-derived next week from different sentences.
...
Kept separate and read carefully. A rejection reason is the most compressed feedback available on
...
|---|---|---|
| `procurement-only-entry` | `approved` | Item `12856969035`, the RFP thesis. Consensus verified from a published county HMIS RFP: 39 agencies, ~329 users, zero occurrences of "case management", "workflow" or "coordinated entry", and a technical spec appendix of three hyperlinks to two HUD documents, the Data Standards Manual and the Data Dictionary. **CORRECTED 2026-08-24: this row previously read "10 of 11 software requirements about reporting", and that was wrong.** Two independent Wrighter runs re-counted the same eleven requirements from the source and agree: **three are about reporting, and exactly one describes the daily work** (requirement 10, "Closed loop referral system", six words with no test attached). Calling browser compatibility a reporting requirement would not survive a reader opening the PDF. The corrected figure is also the stronger claim. Do not restore the old number. **The issuing county is a live prospect, so it is not named in the published piece** — see the organisation-naming rule in `wrighter.md`. |
